    ...
    if (isset($_POST['ids']) && is_array($_POST['ids'])) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $stmt_update = $conn->prepare("UPDATE pressure_handover SET app4 = ?, user = ? WHERE id IN ($idsString)");
                ...
                if ($stmt_update) {
                    ...
                    if ($stmt_update->execute()) {
                        ...
                        header("Location: http://10.11.11.199/dokument-control/approvement/pressure/user/belumapprove.php?username=" . urlencode($username) . "&bagian=" . urlencode($bagian));
                        exit();
                    }
                     else {
